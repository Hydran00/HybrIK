...
# ROS2
# os.chdir("/home/nardi/HybrIK/scripts/src/smpl_fitter/smpl_fitter/")
...
def main():
    ...
    # instantiate camera2smpl node
    rclpy.init()
    camera2smpl = Camera2SMPL(det_model, hybrik_model, img_path, opt, transformation)
    rclpy.spin(camera2smpl)



class Camera2SMPL(Node):

    def __init__(self, det_model, hybrik_model, img_path, opt, transformation):
        super().__init__('smpl_fitter')
        # point cloud
        ...
        self.hybrik_model = hybrik_model
        ...
        # fit callback
        self.timer_ = self.create_timer(0.01, self.fit_smpl)
        self.last_already_processed = False
        # visualizer
        ...
        self.param = None
        self.image = None
        print("Camera2SMPL Node Initialized!!")
        time.sleep(1)
    ...
    def fit_smpl(self):
        if self.last_already_processed == False:
            return
        idx = 0
        input_images = []
        bboxs = []
        ...
        dirname = os.path.dirname(self.img_path)
        basename = os.path.basename(self.img_path)

        with torch.no_grad():
            # Run Detection
            input_image = cv2.cvtColor(cv2.imread(self.img_path), cv2.COLOR_BGR2RGB)
            det_input = det_transform(input_image).to(self.opt.gpu)
            ...
            pose_input = pose_input.to(self.opt.gpu)[None, :, :, :]
        ...
        if idx == 0:
            ...
            self.vis.get_view_control().convert_from_pinhole_camera_parameters(param, allow_arbitrary=True)
        idx += 1
        self.last_already_processed = True
